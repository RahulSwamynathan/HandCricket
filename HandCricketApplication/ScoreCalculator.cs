...
namespace HandCricketApplication
{
    abstract class ScoreCalculator : IScoreCalculator
    {
        ...
        public int BattingCount { get => _battingCount; set => _battingCount = value; }
        ...
        public void GetIndividualScore()
        {
            if(Individuals != null)
            {
                ...
                Console.WriteLine($"Highest Run Scorer of the Session is {Individuals.Where(y => y.Value == Individuals.Max(j => j.Value)).Distinct().First().Key} with the Score of {Individuals.Max(x => x.Value)}");
                Console.ResetColor();
            }
        }
